...
module.exports = function(knex) {
	...
	return {
		...
		getOrder: function(orderId,callback) {
			knex('orders').where('id',orderId).asCallback(callback);
		},
		updateOrder: function(orderId,changesObj,callback) {
			knex('orders').update(changesObj).where('id',orderId).asCallback(function(err,res) {
				console.log(res);
				console.log(changesObj);
				callback(err,res);
			});
		},
		getDishes: function(callback) {
			knex('dishes').asCallback(callback);
		},
